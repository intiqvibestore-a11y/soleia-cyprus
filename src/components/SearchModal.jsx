    ...
    >
      ...
      <div className="mx-4 mb-5 bg-white rounded-2xl border border-[#E8E0D8] shadow-[0_2px_12px_rgba(28,25,23,0.07)] shrink-0">
        ...
        <div className="flex items-center gap-3 px-4 py-4">
          ...
          />
          {treatment && (
            <button onClick={() => setTreatment('')} className="shrink-0 w-5 h-5 rounded-full bg-[#E8E0D8] flex items-center justify-center">
        ...
        <div className="flex items-center gap-3 px-4 py-4">
          ...
          />
          {location && (
            <button onClick={() => setLocation('')} className="shrink-0 w-5 h-5 rounded-full bg-[#E8E0D8] flex items-center justify-center">
              <X className="w-3 h-3 text-[#78716C]" />
            </button>
          )}
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-28">
        ...
        {recents.length > 0 && (
          <div className="mb-6">
            <div className="flex items-center justify-between mb-1">
              <h3 className="text-[12px] font-semibold uppercase tracking-wider text-[#A8A29E]">
                {T.search_recent}
              </h3>
              <button
            ...
            <div className="mt-2 flex flex-col">
              {recents.map((r, i) => (
                <button
                  key={i}
                  onClick={() => doSearch(r)}
                  className="flex items-center gap-3 w-full py-3 border-b border-[#F5F0EB] last:border-none text-left hover:bg-white/60 rounded-lg transition-colors px-1"
                >
        ...
        {/* Browse by category */}
        <h3 className="text-[12px] font-semibold uppercase tracking-wider text-[#A8A29E] mb-3">
          {T.search_categories}
        </h3>
        <div className="grid grid-cols-2 gap-2">
          {CATEGORIES.map(({ name, icon }) => (
            <button
              ...
              className="flex items-center gap-3 px-3 py-3.5 rounded-2xl bg-white border border-[#E8E0D8] hover:border-[#C9A882] hover:bg-[#FDF8F2] transition-all text-left"
            >
      ...
      {/* Fixed search button */}
      <div
        className="fixed bottom-0 left-0 right-0 px-4 pt-3 pb-4 bg-[#FDFAF7] border-t border-[#F0EAE3]"
