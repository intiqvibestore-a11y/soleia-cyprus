...
// Renders Apple Pay / Google Pay button if the browser supports it.
// Must be a child of <Elements> to access useStripe().
function ApplePayButton({ onSuccess }) {
  ...
  useEffect(() => {
    ...
    const paymentRequest = stripe.paymentRequest({
      country: 'CY',
      currency: 'eur',
      // €0.01 — required by Stripe; we never confirm a PaymentIntent so the card
      // is saved but no charge occurs. The amount is shown in the native sheet.
    ...
    <Elements stripe={stripePromise}>
      <div className="min-h-screen flex flex-col">
        ...
        {showAddSheet && (
          ...
          >
            <div className="bg-white px-5 pb-8 pt-5" style={{ borderRadius: '20px 20px 0 0' }}>
              {/* Sheet header */}
              ...
              {/* Option 2: Apple Pay / Google Pay (only if browser supports it) */}
              <ApplePayButton
                onSuccess={() => {
